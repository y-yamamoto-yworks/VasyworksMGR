...
class DeleteBuildingPictureView(FormView):
    """
    建物画像削除
    """
    model = BuildingPicture
    form_class = DeleteForm
    template_name = 'property/delete_building_picture.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        id = kwargs['id']
        self.building_picture = get_object_or_404(BuildingPicture, pk=id, building=building)

        self.kwargs['pk'] = self.building_picture.pk
    ...
    def get_context_data(self, **kwargs):
        ...
        context['data'] = self.building_picture
        return context

    def form_valid(self, form):
        if settings.DEMO:
            messages.error(self.request, 'DEMOモードのため削除できません。')
        elif self.building_picture:
            self.building_picture.is_deleted = True
            self.building_picture.updated_at = timezone.datetime.now()
            self.building_picture.updated_user = self.user
            self.building_picture.save()
    ...
    def get_success_url(self):
        ...
        if self.building_picture:
            if self.back_url:
                ...
                url = reverse_lazy('property_building', kwargs={'oid': self.building_picture.building.oid})
