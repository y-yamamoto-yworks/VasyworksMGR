...
class CreateVacancyInputCleaningView(FormView):
    """
    空室入力退去時清掃作成
    """
    form_class = CreateWithTextForm
    template_name = 'vacancy_item/create_vacancy_input.html'
    success_url = reverse_lazy('vacancy_item_vacancy_input_cleaning_list')
    user = None
    ...
    def get_success_url(self):
        ...
        if self.vacancy_input_item:
            url = reverse_lazy('vacancy_item_vacancy_input_cleaning', kwargs={'idb64': self.vacancy_input_item.idb64})
            back_url = xstr(reverse_lazy('vacancy_item_vacancy_input_cleaning_list'))
    ...
    def get_context_data(self, **kwargs):
        ...
        context['item_name'] = '退去時清掃入力テキスト'
        context['action_url'] = '/vacancy_item/create_input_cleaning/'
        return context
