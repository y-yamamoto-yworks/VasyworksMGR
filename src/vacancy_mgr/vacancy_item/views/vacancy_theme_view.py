...
from vacancy_item.forms import VacancyThemeForm


class VacancyThemeView(UpdateView):
    """
    空室情報テーマ
    """
    model = VacancyTheme
    ...
    def dispatch(self, *args, **kwargs):
        ...
        try:
            ...
            if idb64.isdecimal():
                vacancy_theme_id = xint(idb64)
        except ValueError:
            raise Http404

        self.vacancy_theme = get_object_or_404(VacancyTheme, pk=vacancy_theme_id)

        self.kwargs['pk'] = vacancy_theme_id

        return super().dispatch(*args, **kwargs)

    def get_template_names(self):
        if self.vacancy_theme is None:
            return '404.html'
        elif self.user is None:
            return '404.html'
        elif not self.user.is_company_admin:
            return '404.html'
